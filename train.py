...
def train(train_dataloader, model, opt, epoch, args, writer):
    ...
    epoch_loss = 0
    tqdm_dataloader = tqdm(train_dataloader, desc=f"Epoch {epoch}", leave=False)
...
def test(test_dataloader, model, epoch, args, writer):
    ...
    # Evaluation in Segmentation Task
        ...
        for batch in test_dataloader:
            ...
            with torch.no_grad():     
                pred_labels = model(point_clouds)
                predictions_flat = pred_labels.reshape([-1, args.num_seg_class])
                ...
                # predictions_flat = pred_labels.reshape([-1, args.num_seg_class])
                # print("pred_labels size before reshape:", pred_labels.size())
        ...
        validation_seg_loss = total_loss / len(test_dataloader)

    writer.add_scalar("test_acc", accuracy, epoch)
    if (args.task == "cls"):
        return accuracy
    else:
        return accuracy, validation_seg_loss
...
def main(args):
    ...
    if args.task == "cls":
        ...
        model = seg_model(args, num_seg_classes=args.num_seg_class)
        model.to(args.device)
        if torch.cuda.device_count() > 1:
            print("Using", torch.cuda.device_count(), "GPUs!")
            model = torch.nn.DataParallel(model)
    ...
    if torch.cuda.device_count() > 1:
        opt = optim.Adam(model.module.parameters(), args.lr, betas=(0.9, 0.999))
    if (args.task == "seg"):
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, factor=0.67, patience=10, verbose=True, min_lr = 1e-8)
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.task == "cls":
            ...
            current_acc, seg_valid_loss = test(test_dataloader, model, epoch, args, writer)
            scheduler.step(seg_valid_loss)
...
if __name__ == '__main__':
    ...
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.67, patience=10, verbose=True, min_lr = 1e-8)
